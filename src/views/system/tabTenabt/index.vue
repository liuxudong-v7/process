...
import { fetchTabtenantList, createTabtenant, updaTabtenabt, deleteTabtenabt } from '@/api/sys'
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        remark: [{ required: true, message: 'timestamp is required', trigger: 'change' }],
        level: [{ required: true, message: 'title is required', trigger: 'blur' }]
      },
      tableKey: 0
    }
  },
  ...
  methods: {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updaTabtenabt(tempData.id, tempData).then(() => {
            this.dialogFormVisible = false
    ...
    openMessage() {
      this.$confirm('此操作将永久删除该条信息, 是否继续?', '提示', {
        ...
        if (action === 'confirm') {
          deleteTabtenabt(this.temp.id).then(() => {
            const index = this.dataSource.findIndex(item => item.id === this.temp.id)
